...
def test_dashboard_landing_page(client):
    ...
    assert 'href="/api/docs"' in body  # links to the API docs


def test_check_records_current_version(client):
    resp = client.post("/api/sources/myrepo/mysrc/check", headers=ADMIN)
    assert resp.status_code == 200
    body = resp.json()
    ...
    assert client.get("/api/sources/myrepo/mysrc").json()["current_version"] == "2024-10"


def test_get_returns_pending_without_checking(client):
    body = client.get("/api/sources/myrepo/mysrc").json()
    assert body["status"] == "pending"
    assert body["current_version"] is None


def test_acknowledge_endpoint_removed(client):
    assert client.post("/api/sources/myrepo/mysrc/acknowledge").status_code == 404


def test_unknown_source_404(client):
    assert client.get("/api/sources/nope/nope").status_code == 404
    assert client.post("/api/sources/nope/nope/check", headers=ADMIN).status_code == 404


def test_refresh_all(client):
    resp = client.post("/api/admin/refresh", headers=ADMIN)
    assert resp.status_code == 200
...
def test_reads_are_public(client):
    for path in ("/api/health", "/api/catalog", "/api/sources", "/api/sources/myrepo/mysrc"):
        assert client.get(path).status_code == 200  # no token needed


def test_admin_requires_valid_token(client):
    assert client.post("/api/admin/refresh").status_code == 401  # missing
    assert (
        client.post("/api/admin/refresh", headers={"Authorization": "Bearer nope"}).status_code
        == 401  # wrong
    )
    assert client.post("/api/admin/refresh", headers=ADMIN).status_code == 200
    # X-Admin-Token header is also accepted (use refresh; sync would rebuild the
    # injected test catalog via a real git sync)
    assert (
        client.post("/api/admin/refresh", headers={"X-Admin-Token": "test-token"}).status_code
        == 200
    )
    # ?refresh=true is admin-gated too
    assert client.get("/api/sources/myrepo/mysrc?refresh=true").status_code == 401
    assert (
        client.get("/api/sources/myrepo/mysrc?refresh=true", headers=ADMIN).status_code == 200
    )
...
def test_default_admin_token_enables_admin(tmp_path, monkeypatch):
    ...
    try:
        assert c.get("/api/health").json()["admin_enabled"] is True
        assert c.post("/api/admin/refresh").status_code == 401  # token still required
        assert (
            c.post("/api/admin/refresh", headers={"Authorization": "Bearer changeme"}).status_code
            == 200
        )
    finally:
        c.__exit__(None, None, None)
...
def test_admin_disabled_when_no_token(tmp_path, monkeypatch):
    ...
    try:
        assert c.get("/api/health").status_code == 200  # reads still work
        assert c.get("/api/health").json()["admin_enabled"] is False
        assert c.post("/api/admin/refresh").status_code == 403
        assert c.post("/api/admin/refresh", headers=ADMIN).status_code == 403  # still off
        assert c.post("/api/sources/myrepo/mysrc/check").status_code == 403
    finally:
        c.__exit__(None, None, None)
